...
def argocd(Map config, String image, String repo, String path, String appName, String sshKeyFile, String containerRepository) {
    ...
    if ( config.b_config.containsKey("argocd")
        ...
        withCredentials([string(credentialsId: config.b_config.argocd[config.environment].tokenID, variable: 'TOKEN')]) {
            sh """
            argocd app sync ${path.split('/')[1]} --project ${config.container_repo} --force --insecure --grpc-web --server ${config.b_config.argocd[config.environment].url} --auth-token $TOKEN || true
            argocd app wait ${path.split('/')[1]} --project ${config.container_repo} --timeout 600 --insecure --grpc-web --server ${config.b_config.argocd[config.environment].url} --auth-token $TOKEN
            """
        }
    }
}